...
const I18N = {
  en: {
    ...
    cmp_title:'Compare Robots', cmp_empty:'No robots selected. Add robots from the catalog to compare.', cmp_export:'Export CSV', share_link:'Copy link', copied:'Link copied', diff_only:'Differences only', cmp_add:'Add robot',
    diff_hl:'Highlight differences', diff_badge_ne:'differs', diff_badge_eq:'same', diff_best:'Best', diff_worst:'Weakest',
    diff_stat:'{d} of {n} params differ', diff_none:'All selected robots share identical values', diff_gap:'gap', cmp_select_first:'Select robots to share', cmp_max:'You can compare up to 6 robots', cmp_radar:'Radar chart', drag_sort:'Drag to reorder', swipe_hint:'← Swipe to see all columns →', cmp_search:'Search parameters…', cmp_g_all:'All', cmp_g_spec:'Specs', cmp_g_price:'Price', cmp_g_info:'Info', cmp_g_apps:'Applications', cmp_print:'Print / PDF', cmp_print_sub:'Robot specification comparison', cmp_print_foot:'Generated by RobotHub · Global Robotics Product Library', cmp_print_date:'Generated',
  ...
  zh: {
    ...
    diff_hl:'差异高亮', diff_badge_ne:'有差异', diff_badge_eq:'相同', diff_best:'最优', diff_worst:'最弱',
    diff_stat:'{n} 项参数中 {d} 项存在差异', diff_none:'所选机器人的参数完全一致', diff_gap:'差距', cmp_select_first:'请先选择机器人', cmp_max:'最多对比 6 款机器人', cmp_radar:'雷达图', drag_sort:'拖拽排序', swipe_hint:'← 左右滑动查看全部列 →', cmp_search:'搜索参数…', cmp_g_all:'全部', cmp_g_spec:'规格', cmp_g_price:'价格', cmp_g_info:'基本信息', cmp_g_apps:'应用', cmp_print:'打印 / PDF', cmp_print_sub:'机器人参数对比', cmp_print_foot:'由 RobotHub 生成 · 全球机器人产品库', cmp_print_date:'生成时间',
...
function ensureSelBar(){
  ...
  bar.querySelector('#sbShare').addEventListener('click',shareSelLink);
...
function shareSelLink(){
  const ids=[...getSel()];
  if(!ids.length){ toast(LANG==='zh'?'请先选择机器人':'Select items first','err'); return; }
  const url=location.origin+location.pathname+'?sel='+encodeURIComponent(JSON.stringify(ids));
  copyText(url, ok=>{
    if(ok) toast(LANG==='zh'?'分享链接已复制':'Share link copied','ok');
    else toast((LANG==='zh'?'复制失败：':'Copy failed: ')+url,'err');
  });
}
function copyText(text,cb){
  try{
    if(navigator.clipboard && navigator.clipboard.writeText){
      navigator.clipboard.writeText(text).then(()=>cb&&cb(true)).catch(()=>fallbackCopy(text,cb));
    } else fallbackCopy(text,cb);
  }catch(e){ fallbackCopy(text,cb); }
}
function fallbackCopy(text,cb){
  try{
    ...
    document.body.appendChild(ta); ta.select(); const ok=document.execCommand && document.execCommand('copy'); document.body.removeChild(ta);
    cb&&cb(!!ok);
  }catch(e){ cb&&cb(false); }
}
// 从分享链接 ?sel=[...] 恢复跨页选择篮，并清理 URL 避免刷新重复恢复
function applySelFromUrl(){
  try{
    ...
    if(Array.isArray(arr) && arr.length){
      ...
      toast(LANG==='zh'?`已从链接恢复 ${arr.length} 项选择`:`Restored ${arr.length} selections`,'ok');
    }
  }catch(e){}
}
// ---- 已选清单：按类目/品牌分组折叠显示 ----
